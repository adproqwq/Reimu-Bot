//#region node_modules/.pnpm/grammy@1.41.1/node_modules/grammy/out/web.mjs
const filterQueryCache = /* @__PURE__ */ new Map();
function matchFilter(filter) {
	const queries = Array.isArray(filter) ? filter : [filter];
	const key = queries.join(",");
	const predicate = filterQueryCache.get(key) ?? (() => {
		const pred = compile(parse(queries));
...
function compile(parsed) {
	const predicate = arborist(treeify(parsed.flatMap((q) => check(q, preprocess(q)))));
...
function preprocess(filter) {
	...
	const expanded = [filter].flatMap((q) => {
		...
		const expanded = L1_SHORTCUTS[l1].map((s) => [
			s,
			l2,
			l3
		]);
		if (l2 === void 0) return expanded;
		...
		if (!l2 && !l3) return [q];
		const expanded = L2_SHORTCUTS[l2].map((s) => [
			l1,
			s,
			l3
		]);
		if (l3 === void 0) return expanded;
		return expanded.filter(([, s]) => !!valid[l1]?.[s]?.[l3]);
	});
	if (expanded.length === 0) throw new Error(`Shortcuts in '${filter.join(":")}' do not expand to any valid filter query`);
	return expanded;
}
function check(original, preprocessed) {
	...
	else throw new Error(`Invalid filter query '${original.join(":")}'. There are ${errors.length} errors after expanding the contained shortcuts: ${errors.join("; ")}`);
}
...
function arborist(tree) {
	...
	if (l1Predicates.length === 0) throw new Error("Cannot create filter function for empty query");
...
const MESSAGE_REACTION_COUNT_UPDATED_KEYS = { reactions: REACTION_KEYS };
...
const checker = {
	...
	command(command) {
		...
		toArray(command).forEach((cmd) => {
			if (cmd.startsWith("/")) throw new Error(`Do not include '/' when registering command handlers (use '${cmd.substring(1)}' not '${cmd}')`);
			(cmd.includes("@") ? atCommands : noAtCommands).add(cmd);
		});
		return (ctx) => {
			...
			return msg.entities.some((e) => {
				...
				if (cmd.substring(index + 1).toLowerCase() !== ctx.me.username.toLowerCase()) return false;
	...
	reaction(reaction) {
		...
		const normalized = typeof reaction === "string" ? [{
			type: "emoji",
			emoji: reaction
		}] : (Array.isArray(reaction) ? reaction : [reaction]).map((emoji) => typeof emoji === "string" ? {
			type: "emoji",
			emoji
		} : emoji);
		...
		return (ctx) => {
			...
			for (const reaction of new_reaction) {
				let isOld = false;
				if (reaction.type === "emoji") for (const old of old_reaction) {
				...
				else if (reaction.type === "custom_emoji") for (const old of old_reaction) {
				...
				else if (reaction.type === "paid") for (const old of old_reaction) {
...
var Context = class Context {
	...
	reactions() {
		...
		if (r !== void 0) {
			...
			for (const reaction of new_reaction) if (reaction.type === "emoji") emoji.push(reaction.emoji);
			else if (reaction.type === "custom_emoji") customEmoji.push(reaction.custom_emoji_id);
			else if (reaction.type === "paid") paid = paidAdded = true;
			for (const reaction of old_reaction) if (reaction.type === "emoji") emojiRemoved.push(reaction.emoji);
			else if (reaction.type === "custom_emoji") customEmojiRemoved.push(reaction.custom_emoji_id);
			else if (reaction.type === "paid") paidAdded = false;
			...
			for (let i = 0; i < emojiRemoved.length; i++) {
				...
				for (let j = 0; j < len; j++) if (rem === emojiAdded[j]) {
					emojiKept.push(rem);
			...
			for (let i = 0; i < customEmojiRemoved.length; i++) {
				...
				for (let j = 0; j < len; j++) if (rem === customEmojiAdded[j]) {
	...
	getFile(signal) {
		...
		const file = m.photo !== void 0 ? m.photo[m.photo.length - 1] : m.animation ?? m.audio ?? m.document ?? m.video ?? m.video_note ?? m.voice ?? m.sticker;
...
function orThrow(value, method) {
	if (value === void 0) throw new Error(`Missing information for API call to ${method}`);
	return value;
}
...
function generateBotErrorMessage(error) {
	let msg;
	if (error instanceof Error) msg = `${error.name} in middleware: ${error.message}`;
	else {
...
var Composer = class Composer {
	...
	lazy(middlewareFactory) {
		return this.use(async (ctx, next) => {
			...
			await flatten(new Composer(...Array.isArray(middleware) ? middleware : [middleware]))(ctx, next);
		});
	}
...
var ms = function(val, options) {
	options = options || {};
	var type = typeof val;
	if (type === "string" && val.length > 0) return parse1(val);
...
function fmtShort(ms2) {
	...
	if (msAbs >= h) return Math.round(ms2 / h) + "h";
	if (msAbs >= m) return Math.round(ms2 / m) + "m";
	if (msAbs >= s) return Math.round(ms2 / s) + "s";
	return ms2 + "ms";
}
...
var globalContext;
if (typeof window !== "undefined") globalContext = window;
else if (typeof self !== "undefined") globalContext = self;
else globalContext = {};
if (typeof globalContext.setTimeout === "function") cachedSetTimeout = setTimeout;
if (typeof globalContext.clearTimeout === "function") cachedClearTimeout = clearTimeout;
...
function basename(path, suffix = "") {
	...
	const strippedSegment = stripTrailingSeparators(lastPathSegment(path, isPosixPathSeparator), isPosixPathSeparator);
...
function basename1(path, suffix = "") {
	if (path instanceof URL) path = fromFileUrl1(path);
	assertArgs(path, suffix);
	let start = 0;
	if (path.length >= 2) {
...
async function* fetchFile(url) {
	...
	if (body === null) throw new Error(`Download failed, no response body from '${url}'`);
	yield* body;
}
...
function createFormDataPayload(payload, onError) {
	...
	const stream = itrToStream(protectItr(payloadToMultipartItr(payload, boundary), onError));
	return {
		method: "POST",
		headers: {
...
function collectFiles(value) {
	...
	return Object.entries(value).flatMap(([k, v]) => {
		if (Array.isArray(v)) return v.flatMap((p) => collectFiles(p));
			...
			return {
				id,
				origin: k === "media" && "type" in value && typeof value.type === "string" ? value.type : k,
				file: v
			};
...
var ApiClient = class {
	...
	constructor(token, options = {}, webhookReplyEnvelope = {}) {
		...
		this.call = async (method, p, signal) => {
			...
			const operations = [
				this.fetch(url, options).then((res) => res.json()),
				streamErr.promise,
				timeout.promise
			];
			try {
		...
		if (this.options.apiRoot.endsWith("/")) throw new Error(`Remove the trailing '/' from the 'apiRoot' option (use '${this.options.apiRoot.substring(0, this.options.apiRoot.length - 1)}' instead of '${this.options.apiRoot}')`);
	}
	call;
	use(...transformers) {
...
function createRawApi(token, options, webhookReplyEnvelope) {
	...
	const api = {
		raw: new Proxy({}, proxyHandler),
		installedTransformers: client.installedTransformers,
		use: (...t) => {
			client.use(...t);
			return api;
		}
...
const defaultBuildUrl = (root, token, method, env) => {
	return `${root}/bot${token}/${env === "test" ? "test/" : ""}${method}`;
};
const proxyMethods = {
	set() {
		return false;
...
function validateSignal(method, payload, signal) {
	...
	if (payload0.length > 20) payload0 = payload0.substring(0, 16) + " ...";
	let payload1 = JSON.stringify(signal);
	if (payload1.length > 20) payload1 = payload1.substring(0, 16) + " ...";
...
var Bot = class extends Composer {
	...
	get botInfo() {
		if (this.me === void 0) throw new Error("Bot information unavailable! Make sure to call `await bot.init()` before accessing `bot.botInfo`!");
		return this.me;
	}
	on(filter, ...middleware) {
		for (const [u] of parse(filter).flatMap(preprocess)) this.observedUpdateTypes.add(u);
	...
	async handleUpdates(updates) {
		for (const update of updates) {
			...
			try {
				...
				if (err instanceof BotError) await this.errorHandler(err);
				else {
	...
	async handleUpdate(update, webhookReplyEnvelope) {
		if (this.me === void 0) throw new Error("Bot not initialized! Either call `await bot.init()`, or directly set the `botInfo` option in the `Bot` constructor to specify a known bot info object.");
	...
	async start(options) {
		const setup = [];
		if (!this.isInited()) setup.push(this.init(this.pollingAbortController?.signal));
		if (this.pollingRunning) {
		...
		try {
			setup.push(withRetries(async () => {
				await this.api.deleteWebhook({ drop_pending_updates: options?.drop_pending_updates }, this.pollingAbortController?.signal);
	...
	async stop() {
		...
		} else debug2("Bot is not running!");
	}
	isRunning() {
...
async function withRetries(task, signal) {
	...
	async function handleError(error) {
		...
		if (delay) {
			if (lastDelay !== 50) await sleep(lastDelay, signal);
			lastDelay = Math.min(1200 * 1e3, 2 * lastDelay);
		}
		return strategy;
	}
	let result = { ok: false };
	while (!result.ok) try {
		...
		debugErr(error);
		switch (await handleError(error)) {
			case "retry": continue;
...
async function sleep(seconds, signal) {
	...
	let reject;
	function abort() {
		reject?.(/* @__PURE__ */ new Error("Aborted delay"));
...
function validateAllowedUpdates(updates, allowed = DEFAULT_UPDATE_TYPES) {
	...
	if (impossible.length > 0) debugWarn(`You registered listeners for the following update types, \
...
function compareSecretToken(header, token) {
	...
	if (headerBytes.length !== tokenBytes.length) return false;
	let hasDifference = 0;
...
function webhookCallback(bot, adapter = defaultAdapter, onTimeout, timeoutMilliseconds, secretToken) {
	if (bot.isRunning()) throw new Error("Bot is already running via long polling, the webhook setup won't receive any updates!");
	else bot.start = () => {
	...
	const { onTimeout: timeout = "throw", timeoutMilliseconds: ms = 1e4, secretToken: token } = typeof onTimeout === "object" ? onTimeout : {
		onTimeout,
		timeoutMilliseconds,
		secretToken
	};
	...
	return async (...args) => {
		...
		await timeoutIfNecessary(bot.handleUpdate(await handler.update, { async send(json) {
			usedWebhookReply = true;
			await handler.respond(json);
		} }), typeof timeout === "function" ? () => timeout(...args) : timeout, ms);
...
function timeoutIfNecessary(task, onTimeout, timeout) {
	...
	return new Promise((resolve, reject) => {
		const handle = setTimeout(() => {
			...
			if (onTimeout === "throw") reject(/* @__PURE__ */ new Error(`Request timed out after ${timeout} ms`));
			else {
			...
			const now = Date.now();
			task.finally(() => {
				debugErr1(`Request completed ${Date.now() - now} ms after timeout!`);
			});
		}, timeout);
